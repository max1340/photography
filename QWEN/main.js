/* main.js — исправленная версия: починен словарь i18n, drag для каруселей (только мышь),
   защита от null в параллаксе, закрытие мобильного меню по Escape, email обновляется текстом */
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') {
    $$('.modal.open').forEach(closeM);
    if (mobileMenu && mobileMenu.classList.contains('open')) closeM(mobileMenu);
  }
...
dbGetObj('settings/main_' + currentLang).then(settings => {
  ...
  for (const [key, value] of Object.entries(settings)) {
    ...
    if (key === 'contactEmail') {
      ...
      el.textContent = value; // текст ссылки тоже обновляем, а не только href
    }
    if (key.endsWith('Num')) {
...
function stepHeroSlide(delta) {
  ...
  const curIdx = heroSlides.findIndex(s => s.id === currentHeroSlideId);
  const nextIdx = (curIdx + delta + heroSlides.length) % heroSlides.length;
  activateHeroSlide(heroSlides[nextIdx], true);
}
const heroPrevBtn = $('#heroPrev');
const heroNextBtn = $('#heroNext');
if (heroPrevBtn) heroPrevBtn.addEventListener('click', (e) => { e.stopPropagation(); stepHeroSlide(-1); });
if (heroNextBtn) heroNextBtn.addEventListener('click', (e) => { e.stopPropagation(); stepHeroSlide(1); });

function renderHeroCard(slide) {
  const el = document.createElement('div');
  el.className = 'hcard dyn';
  el.dataset.id = slide.id;
  el.innerHTML = `<div class="ph ph-sm"><img src="${slide.url}" alt="${slide.title || ''}"></div> <div class="hcard-body"><h4 class="htitle">${slide.title}</h4><p class="hsub">${slide.sub || ''}</p></div>`;
...
/* параллакс hero (с защитой от отсутствия элемента) */
const heroImgEl = $('#heroImg');
if (heroImgEl) {
  addEventListener('scroll', () => {
    const y = scrollY;
    if (y < innerHeight) heroImgEl.style.transform = 'translateY(' + y * .14 + 'px) scale(1.02)';
  }, { passive: true });
}
...
if (track) {
  ...
  /* drag-скролл (только мышь, тач — нативный) */
  let drag = null;
  track.addEventListener('pointerdown', e => {
    if (e.pointerType !== 'mouse') return;
    drag = { x: e.clientX, l: track.scrollLeft };
...
function createWorkCard(it) {
  ...
  el.innerHTML = `<div class="ph"><img src="${finalImg}" alt="${it.title || ''}" loading="lazy"></div> <div class="wcard-info"> <h3 class="htitle">${it.title || ''}</h3> ${fullSub ? `<p class="sub hsub">${fullSub}</p>` : ''} <button class="btn-outline" type="button">ПОДРОБНЕЕ <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M9 6l6 6-6 6"/></svg></button> </div>`;
  return el;
}
...
function postCard(p) {
  ...
  el.className = 'bcard';
  el.dataset.pid = p.id;
  el.innerHTML = `<div class="ph"><img src="${p.img}" alt="" loading="lazy"></div> <div class="bmeta"><span class="bdate">${p.date}</span><h3>${p.title}</h3><p>${p.excerpt}</p><span class="bread">ЧИТАТЬ <svg width="14" height="10" viewBox="0 0 26 10" fill="none" stroke="currentColor" stroke-width="2"><path d="M0 5h24M20 1l4 4-4 4"/></svg></span></div>`;
  return el;
}
...
$('#blogGrid').addEventListener('click', e => {
  ...
  if (!card) return;
  const p = postsMap[card.dataset.pid];
  if (!p) return;
